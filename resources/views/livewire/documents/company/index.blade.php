                            ...
                            @forelse($documents as $document)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <a href="{{ Storage::url($document->document_file) }}" target="_blank" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
    ...
    <x-modal wire:model="showEditModal">
        ...
        <x-slot name="content">
            <form wire:submit="update">
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        <label for="edit_document_type_id" class="block text-sm font-medium text-gray-700">Document Type</label>
                        <select id="edit_document_type_id" wire:model="document_type_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            @foreach($documentTypes as $key => $label)
                                <option value="{{ $key }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="edit_title" wire:model="title" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        @error('title') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label for="edit_issue_date" class="block text-sm font-medium text-gray-700">Issue Date</label>
                        <input type="date" id="edit_issue_date" wire:model="issue_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        @error('issue_date') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        <label for="edit_document_file" class="block text-sm font-medium text-gray-700">Document File</label>
                        <input type="file" id="edit_document_file" wire:model="document_file" class="mt-1 block w-full text-sm text-gray-500
                            file:mr-4 file:py-2 file:px-4
                            file:rounded-md file:border-0
                            file:text-sm file:font-semibold
                            file:bg-indigo-50 file:text-indigo-700
                            hover:file:bg-indigo-100">
                        @error('document_file') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        @if($editingDocument && $editingDocument->document_file)
                            <div class="mt-2">
                                <a href="{{ Storage::url($editingDocument->document_file) }}" target="_blank" class="text-sm text-indigo-600 hover:text-indigo-900">View current file</a>
                            </div>
                        @endif
                    </div>
